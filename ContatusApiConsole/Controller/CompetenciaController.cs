...
{
    internal class CompetenciaController
    {
        public async Task<int> PostTotalItems()
        {
            ...
            var body = new
            {
                competenceDateFrom = dataInicio,
                competenceDateTo = dataFim,
                quickFilter = "ALL",
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var url = "https://services.contaazul.com/finance-pro-reader/v1/competence-view";
        ...
        public async Task<List<Competencia>> PostCompetencia(int pagina)
        {
            //int pagina = 1; // Substitua pelo valor desejado
            int tamanhoPagina = 1000; // Substitua pelo valor desejado
            DateTime dataInicial = DateTime.Parse("2021-01-01"); // Substitua pelo valor desejado
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (itemsToken != null)
                    {
                        var competencia = itemsToken.ToObject<List<Competencia>>();
